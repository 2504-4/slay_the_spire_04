...
public class GameView {
    ...
    public void update() {
        ...
        // 手牌 / 小怪奖励（二者复用同一区域，避免改变原有界面布局）
        handBox.getChildren().clear();
        if (controller.isRewardPending()) {
            handTitle.setText("🎁 选择一张奖励卡加入牌组");
            for (int i = 0; i < controller.getRewardChoices().size(); i++) {
                handBox.getChildren().add(createRewardCardButton(controller.getRewardChoices().get(i), i));
            }
        } else {
            handTitle.setText("你的手牌");
        ...
        boolean actionLocked = over || controller.isRewardPending();
        endTurnButton.setDisable(actionLocked);
        endTurnButton.setStyle(actionLocked ? BTN_END_DISABLED : BTN_END_NORMAL);
        if (over) {
    ...
    /** 奖励卡按钮不消耗能量，点击后直接由 Controller 处理选卡与关卡推进。 */
    private Button createRewardCardButton(GameModel.Card card, int index) {
        Button button = new Button();
        button.setPrefWidth(CARD_WIDTH + 20);
        button.setPrefHeight(CARD_HEIGHT);
        button.setStyle(card.getType() == GameModel.Card.CardType.ATTACK
                ? CARD_ATTACK_STYLE : CARD_SKILL_STYLE);

        VBox content = new VBox(7);
        content.setAlignment(Pos.CENTER);
        Label nameLabel = new Label(card.getName());
        nameLabel.setFont(Font.font("Microsoft YaHei", FontWeight.BOLD, 17));
        nameLabel.setTextFill(Color.WHITE);
        Label costLabel = new Label("⚡ " + card.getCost() + " 费");
        costLabel.setFont(Font.font("Arial", FontWeight.BOLD, 14));
        costLabel.setTextFill(Color.GOLD);
        Label descLabel = new Label(card.getDescription());
        ...
        descLabel.setTextFill(Color.WHITE);
        descLabel.setWrapText(true);
        descLabel.setMaxWidth(CARD_WIDTH);
        content.getChildren().addAll(nameLabel, costLabel, descLabel);
        button.setGraphic(content);
        ...
        button.setOnMouseEntered(e -> {
            button.setScaleX(1.05);
            button.setScaleY(1.05);
            button.setEffect(new DropShadow(20, Color.GOLD));
        });
        button.setOnMouseExited(e -> {
            button.setScaleX(1.0);
            button.setScaleY(1.0);
